...
public class CardManager {
	 ...
	 * initializeCards()
	 * saveCards()
	 * saveNewCard(String cardNumber, int colorIndex)
	 * addNewCard(String cardNumber, int idx)
	 * deleteCard(final String cardNumber)
	...
	public static void initializeCards() {
		...
		if (current.length() == 1) {
			...
			for (int i = 1; i < cards.length; i++) {
				...
				MainActivity.activeCards.add(new Card(cardData[0], Integer.parseInt(cardData[1]), def));
			}
		}		
	}
	
	//Loops through the array list of cards and saves the data to a string to be stored in System Prefs
	...
	//saves the new card to the system prefs (aka concats the string and saves it to system prefs)
	...
	//Creates a new card and adds it to the arraylist of cards and updates the list view
	...
	public static void deleteCard(final String cardNumber) {
		...
		if (MainActivity.activeCards.size() == 1) {
			MainActivity.showDeleteError("Error", "Can't remove your only card!");
		} else {
	...
	public static void setDefault(Card card) {
		...
		for(Card d : MainActivity.activeCards) d.setDefault(false);
		...
		new BarcodeAsyncTask(MainActivity.pref.getString(MainActivity.DEFAULTCARD, null), MainActivity.context, MainActivity.systemService).execute();
	}
}
